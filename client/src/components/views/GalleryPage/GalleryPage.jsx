...
function GalleryPage() {
  ...
  const getInstagramPost = () => {
    try {
      axios
        .get(
          `${process.env.REACT_APP_MEDIA_BASE_URL}me/media?fields=id,media_type,media_url,media_count,caption,timestamp&limit=${loadPost}&access_token=${process.env.REACT_APP_INSTA_API_TOKEN}`
        )
  ...
  return (
    <div style={{ padding: "2rem" }}>
      <PageContent title={galleryTitle} desc="👍🏻 스루비룸 팔로우하기" />
      <FeedWrapper>
        {feeds &&
          feeds.map((feed, idx) => (
            <Feed key={`feed${idx}`}>
              ...
              <div className="timestamp">
                {moment(feed.timestamp).format("YYYY.MM.DD")}
              </div>
              <div style={{ padding: "0.7rem" }}>
                {renderCaption(feed.caption)}
              </div>
            </Feed>
          ))}
      </FeedWrapper>
